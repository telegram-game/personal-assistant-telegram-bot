...
export class TelegramService {
    ...
    [
      ...
      },
      {
        ...
        buildMessage: (msg: TelegramBot.Message): string => {
          ...
          const messageId = msg.reply_to_message.text.match(
            this.approveMessageTemplate,
          );
          if (!messageId) {
            return '';
          }
  ...
  private initEvent(
    ...
    this.telegramBot.onText(new RegExp(command), (msg: TelegramBot.Message) => {
      ...
      if (
        options?.permissionCheckingFunc &&
        !options?.permissionCheckingFunc(msg)
      ) {

...
export class TelegramMessageController {
  ...
  async send(@Body() data: TelegramMessagePayload): Promise<void> {
  ...
  @Post('internal')
  @HttpCode(HttpStatus.ACCEPTED)
  async sendInternal(@Body() data: TelegramMessagePayload): Promise<void> {
    await this.telegramMessageService.receiveTelegramMessageInternalProcess(
      data,
    );
  }
